 ...
 *
 * SHARED between the app (`src/lib/beatmaps.ts` re-exports this) and the
 * Cloud Functions (`functions/src`), so it must stay dependency-free: no
 * React, Expo, Firebase or Node imports.
 *
...
/**
 * Content hash of a beatmap: FNV-1a (32-bit, hex) over `serializeBeatmap`, so
 * the app and the server agree on which chart a run was scored against
 * without shipping a hashing library. Not cryptographic — the server holds
 * its own copy of the chart and re-verifies the cue set; the hash only detects
 * a stale/mismatched client bundle.
 */
export function beatmapHash(beatmap: Beatmap): string {
  const text = serializeBeatmap(beatmap);
  let hash = 2166136261;
  for (let index = 0; index < text.length; index += 1) {
    hash ^= text.charCodeAt(index);
    hash = Math.imul(hash, 16777619);
  }
  return `b1-${(hash >>> 0).toString(16).padStart(8, '0')}`;
}
